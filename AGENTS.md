...
`AGENTS.md` is the canonical runtime instruction file. Runtime-specific adapters
(`CLAUDE.md`, `.cursor/rules/insecur.mdc`) should stay short and point back here; keep Codex,
Claude, and Cursor aligned through the skills and the config.
...
- **Worktree env sync:** in a secondary worktree, run
  `pnpm setup:worktree --source /Users/isaacsuttell/src/insecur --skip-install --skip-dev-check`
  before local smoke, preview deploy, or service-auth work. This copies ignored private env files
  from the primary checkout without printing values.
...
- Lefthook pre-commit runs staged Prettier/ESLint, optional local gitleaks (skipped when `gitleaks` is not on PATH), and `turbo typecheck`; pre-push runs `pnpm verify:prepush`, which expands to `pnpm verify:pr` plus `pnpm test:coverage`. That mirrors the PR CI hot path for policy checks, Turbo affected package checks, and the DB-less coverage ratchet; CI's `Verify` job also adds current-tree gitleaks and path-scoped DB-backed tests. `merge_group` and `push` events run full `pnpm verify`. The `security-daily` workflow adds slower security scanners (gitleaks history, semgrep, syft+grype) on Blacksmith runners; those stay CI-only and out of the push hot path (`docs/build-tooling.md`).
...
- M0 contract-and-gate enforcement (2026-06-12 ADR batch) is blocking in CI: the Plaintext Metadata Allowlist conformance gate (ADR-0070) runs in `pnpm verify` and `pnpm test:rls`; the package-boundary conformance gate runs in `verify:policy` to keep public/contract packages off the crypto graph; the role-bundle registry conformance suite (ADR-0034) and machine-only `runtime_injection:grant_issue_protected` atom (ADR-0038) run in `@insecur/access` tests; the `OPERATION_INTENT_CODES` catalog (ADR-0068), `operation.idempotency_mismatch` check (ADR-0066), and non-lease `execution_deadline` claims with lazy abandonment recovery (ADR-0073) run in `@insecur/operations`; the no-plaintext canary gate `pnpm test:canary` (ADR-0069) runs in the DB-backed `Verify` step after `test:e2e`; the exit/HTTP lockstep test (ADR-0062) runs in `@insecur/worker-kit`; the decrypt-import lint boundary (ADR-0071) is enforced via `eslint.config.ts` inside `pnpm verify`.
