...
export async function runConnectionsDisconnectCommand(
  ...
  const result = await api.disconnectAppConnection(target);
  return finishConnectionCommand(
    flags,
    result,
    () => `Disconnected connection ${target.appConnectionId}.`,
  );
}
