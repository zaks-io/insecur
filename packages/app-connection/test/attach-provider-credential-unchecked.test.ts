...
import { attachProviderCredentialUnchecked } from "../src/attach-provider-credential-unchecked.js";
...
describe("attachProviderCredentialUnchecked", () => {
  it("activates the connection with the encrypted credential", async () => {
    ...
    const result = await attachProviderCredentialUnchecked({
      organizationId: ORG,
  ...
  it("fails closed when the app connection does not exist", async () => {
    const appConnectionStore = {
      getConnectionById: vi.fn(async () => null),
      attachActiveProviderCredential: vi.fn(),
    } as unknown as TenantAppConnectionStore;
