...
export interface ScryptSecretVerifierMaterial {
  algorithm: string;
  saltB64: string;
  hashB64: string;
}

export function hashScryptSecret(secret: string): ScryptSecretVerifierMaterial {
...
export function verifyScryptSecret(
  secret: string,
  verifier: ScryptSecretVerifierMaterial,
): boolean {
  if (verifier.algorithm !== SCRYPT_V1_ALGORITHM) {
    return false;
  }
