...
export async function mintDerivedAgentSessionCredential(
  ...
  const ttlSeconds = Math.min(CLI_SESSION_TTL_SECONDS, remainingParentTtl);
  if (ttlSeconds <= 0) {
  ...
  if (input.harnessName !== undefined && input.harnessName.trim() !== "") {
    payload.hrn = input.harnessName.trim();
  }
  return {
