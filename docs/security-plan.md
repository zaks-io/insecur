...
- Treat user-authored product names as Display Names: plaintext-allowlisted ordinary metadata visible according to Metadata Visibility Policy after authentication and authorization.
...
- High-risk human actions require a High-Assurance Challenge: a fresh WorkOS passkey verification at challenge time. TOTP enrollment metadata is not proof that TOTP was freshly verified, so TOTP cannot clear a challenge until the provider adapter supplies explicit per-challenge factor evidence. Login-time MFA, session age, and session attributes never satisfy a High-Assurance Challenge (ADR-0009, ADR-0032).
...
- Display Names are plaintext-allowlisted ordinary metadata, not Sensitive Values or Sensitive Metadata.
...
- Plaintext lookup/index fields are limited to the Plaintext Metadata Allowlist, including opaque resource IDs and Display Names. Approval Context Notes, Approval Rejection Notes, Push Device Registrations, provider target names, provider-side secret or variable names used by Explicit Provider Lookup or Secret Sync Bindings, policy binding names, exact Protected Environment Secret value byte lengths, and security-relevant relationships are encrypted Sensitive Metadata.
...
- Display Names may be used for scoped lookup and list filtering after authorization where Metadata Visibility Policy permits. Sensitive Metadata must not be copied into plaintext search indexes.
- Display Names are unencrypted plaintext metadata, so users must not place Sensitive Values or confidential context in them; the CLI and UI should surface this guidance when a Display Name is created or edited, and Metadata Visibility Policy may hide them from lower-assurance surfaces.
...
- Sensitive Values must enter through safe sensitive input paths such as request bodies over TLS, CLI stdin, masked prompts for human TTY callers, service generation, provider authorization flows, or development-only Secret Import.
- Non-interactive secret writes must explicitly select service generation or stdin input and fail with `secret.input_required` otherwise.
- Sensitive Values must never be accepted in URLs, query strings, route params, CLI arguments, shell-visible flags, named local value files for ordinary secret writes, or GET requests.
...
- Secret Import is separate from Secret Sync reconciliation, is limited to non-protected development adoption, and must use Safe Sensitive Input Paths and audit events.
...
- Protected Environment Promotion and protected rollback require a High-Assurance Challenge.
- Sensitive Values are accepted by stdin, masked prompt for human TTY callers, request body, service generation, provider flow, or development-only Secret Import only; no `--value <secret>`, `--value-file <path>`, `--token <secret>`, or `--client-secret <secret>` inputs.
...
- Secret Import tests prove provider-side Sensitive Values are never read back in V1, import is limited to non-protected development Environments, and imported values enter only through Safe Sensitive Input Paths.
...
- No Plaintext Persistence and Secret-Free Logging tests pass with canary Sensitive Values. The `pnpm test:canary` gate ([ADR-0069](adr/0069-no-plaintext-canary-gate.md)) proves the structurally enumerated surfaces it sweeps: Postgres columns (including operation records and audit rows), in-process console capture, and First Value HTTP/RPC egress. The release gate separately requires external zero-finding evidence for deployed R2 backups, Worker logs, traces, and API analytics, but their provider query/download implementations are not yet present. KV, Queue, and Durable Object surfaces are not deployed.
