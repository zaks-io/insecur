...
first mutation. A draft bound to a different SHA, or a draft whose marker is missing, is never
updated in place: delete the draft and re-dispatch CLI Release from `main` to rebuild it. These
invariants are covered by `scripts/ci/cli-release-workflow.test.mjs` and
`scripts/ci/cli-release-draft-guard.test.mjs` in `pnpm test:scripts`.
