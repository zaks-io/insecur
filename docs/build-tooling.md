...
Draft source-SHA binding: every draft records its verified source SHA as a durable marker in the
release body (`<!-- insecur-cli-release-source-sha: <sha> -->`, written by
`scripts/ci/cli-release-notes.mjs`). Before any same-version draft mutation (retarget, notes edit,
asset deletion, or `--clobber` upload), `scripts/ci/cli-release-draft-guard.mjs` re-reads the draft
and fails closed unless the recorded marker equals the dispatched source SHA; it runs in the
prepare job (fail fast, before binaries build) and again in the release job immediately before the
first mutation. A draft bound to a different SHA, or a draft whose marker is missing or ambiguous
(any count other than exactly one), is never updated in place: delete the draft and re-dispatch
CLI Release from `main` to rebuild it. The notes pipeline strips or rejects marker text inside
release-note content, so the trailing marker written by the workflow is the only marker a
generated body can contain, and all workflow runs share one static concurrency group so same-tag
dispatches cannot interleave between guard and mutation. These invariants are covered by
`scripts/ci/cli-release-workflow.test.mjs` and `scripts/ci/cli-release-draft-guard.test.mjs` in
`pnpm test:scripts`.
