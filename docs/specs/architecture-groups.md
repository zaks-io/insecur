...
This document breaks the decided product into architectural ownership groups. Each group owns a
small set of seams, references, and handoff contracts. These groups are intentionally allowed to
depend on each other; they are not execution lanes and do not imply that work can run in parallel.

Terminology: `AG0` through `AG10` are durable architecture group IDs. Linear `W*` Workstream labels
are separate execution lanes owned by [workflow/config.md](../agents/workflow/config.md). Do not
use architecture group IDs as Linear Workstream labels.

## Global Rules For All Agents

- Start from [product-spec.md](product-spec.md), then read only the ADRs and area docs linked from
  your architecture group.
...
- When an architecture group needs a new decision, write or amend an ADR, then update the specs before
  continuing.
...
| Seam                            | Owner                         | Contract                                                                                                                                                                                                                                                                   |
| ------------------------------- | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| First Value Milestone Contract  | AG2/AG4/AG5 with AG1/AG3/AG10 | Provider-free non-protected development Secret Use through the real tenant, access, storage, crypto, Secret Version, Runtime Injection, and audit seams.                                                                                                                   |
| Tenant-Scoped Store             | AG1                           | Short scoped transaction, tenant-local RLS setting, no raw executor, audit-safe typed data access.                                                                                                                                                                         |
| Effective Access Resolver       | AG2                           | Actor plus Organization/Project/Environment IDs in, coordinate-bound Authorization Scope set out.                                                                                                                                                                          |
| Keyring                         | AG3                           | Resolve and rewrap key hierarchy without exposing tenant keys to callers.                                                                                                                                                                                                  |
| Encryption Envelope             | AG3                           | Domain identity plus plaintext in, wrapped material out; decrypt only into an approved execution path, the modules on the decrypt-import allowlist decided by [ADR-0071](../adr/0071-decrypt-egress-import-boundary.md). The lint enforcement lives in `eslint.config.ts`. |
| Storage Security Gate           | AG3                           | Readiness verdict and evidence for production Secret Delivery and Secret Sync.                                                                                                                                                                                             |
| Secret Version Store            | AG4                           | Wrapped material and exact IDs only; append, publish, rollback, discard; no approval knowledge.                                                                                                                                                                            |
| Operation Store                 | AG1 with AG8/AG10             | Durable operation state, idempotency, wait/retry metadata, leases, fencing tokens, cancellation, and audit references.                                                                                                                                                     |
| Protected Change Orchestrator   | AG6                           | Metadata-only exact-ID state machine for Promotion, rollback, approval, and policy gates.                                                                                                                                                                                  |
| Provider Adapter Port           | AG8                           | Metadata-only plan/lookup plus approved write execution; no provider value read-back.                                                                                                                                                                                      |
| Runtime Injection Grant Service | AG5 with AG7                  | Short-lived one-use grant issue/consume for exact Runtime Injection Policy Versions or allowed non-protected one-command selections.                                                                                                                                       |
| Audit Event Writer              | AG10                          | Typed tenant-qualified event write with Sensitive Metadata handling and export compatibility.                                                                                                                                                                              |
| Human Approval Surface          | AG9 with AG6                  | Web-mediated challenge and approval UI for bounded operation IDs and impact reviews.                                                                                                                                                                                       |
| Sensitive Detail Gate           | AG2 with AG9                  | High-Assurance-Challenge-backed decision required before decrypted Sensitive Metadata appears in any User-facing surface or full-fidelity export.                                                                                                                          |
| Web-To-API Token                | AG9 with AG2                  | Short-lived scoped token minted per request by the BFF and validated through the Effective Access Resolver; the browser never holds a bearer token.                                                                                                                        |

## Integration Order

1. AG0 establishes the repo, package manager, lint/type/test gates, CI skeleton, and cache posture.
2. AG1, AG2, and AG3 define interface stubs together, then build persistence, access, and crypto in
   parallel against fake adapters where needed.
3. AG4 builds the Secret Version Store and safe write/read-for-use paths on top of AG1 and AG3.
4. AG2, AG4, AG5, and AG10 build the First Value Milestone only through the contract in
   [first-value-milestone.md](../first-value-milestone.md).
5. AG5 builds CLI command framework and non-protected First Value Runtime Injection on AG2/AG4.
6. AG6 and AG9 build Protected Change Orchestration and the Human Approval Surface on AG2/AG4. AG6
   builds the core Promotion approval state machine and High-Assurance Challenge gates first;
   Approval Impact Review ships without sync impact until AG8 lands, consistent with
   [product-spec.md](product-spec.md) section 8, which scopes that impact "where applicable".
7. AG7 adds Machine Identity, GitHub OIDC, and deploy credentials so protected runtime injection has
   a real credential boundary.
8. AG8 adds App Connections and Cloudflare/GitHub sync on AG1/AG3/AG4/AG6/AG10 and wires Provider Adapter
   Port plan/lookup metadata into the Approval Impact Review. Protected Secret Sync enable/run and
   Cloudflare Worker secret writes stay fail-closed until AG6's approval gates exist, because the
...
9. AG10 keeps audit, telemetry, backup, restore, incident response, and release-gate evidence wired
   throughout.

## AG0 - Tooling, CI, And Supply Chain
...
## AG1 - Persistence, Tenant Boundary, And Operations State
...
## AG2 - Human Identity, Authorization, And Onboarding

Owns:

- WorkOS AuthKit integration, session validation primitives and CLI session exchange, CSRF helpers
  consumed by AG9, MFA enrollment state, and High-Assurance Challenge initiation.
- Web-to-API token validation through the Effective Access Resolver for tokens AG9 mints.
...
Does not own web layout, browser-facing session cookie/CSRF/rotation behavior (AG9 per ADR-0051),
...
## AG3 - Key Custody, Keyring, Encryption, And Storage Security Gate
...
## AG4 - Secret Lifecycle And Version Store
...
## AG5 - CLI, Local Config, And Runtime Injection
...
- Protected Environment injection issuance requires the `runtime_injection:grant_issue_protected`
  scope, which only AG7 machine credential resolution contributes, per
...
## AG6 - Protected Changes, Approval, And Delivery Risk Policy
...
provider writes, or Approval Notification delivery adapters (AG10); the orchestrator emits
...
## AG7 - Machine Access, OIDC, And Deploy Credentials
...
- Protected Environment machine credential boundary for Runtime Injection: machine effective-access
  resolution is the only contributor of the `runtime_injection:grant_issue_protected` scope AG5's
...
## AG8 - App Connections, Provider Adapters, And Secret Sync
...
## AG9 - Web BFF, Tenant Console, And Human Approval Surface
...
- Browser-facing session cookie handling, CSRF enforcement, and session rotation per ADR-0051,
  consuming AG2's session validation and CSRF primitives, plus Service Binding calls to the API
  Worker.
- Short-lived scoped web-to-API token minting with no browser-held bearer token; AG2 validates the
...
Does not own Effective Access semantics, AG2's auth and session validation primitives, protected
...
## AG10 - Audit, Telemetry, Backup, Restore, Runbooks, And Claims
...
offboarding. Registration lifecycle likely lands in AG2 with the account security controls it owns;
...
canonical audit event code registry in `packages/audit/src/audit-event-codes.ts`, the same way AG2
...
## Hand-Off Checklist For Any Architecture Group
