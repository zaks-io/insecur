...
During prelaunch, the daily release train selects the newest CI-green `main` commit, deploys and
smokes that exact SHA in Preview, then rebuilds the same source commit with Production bindings
through the protected `Production` GitHub Environment. The `production` branch advances only after
the live fleet and release evidence verify that SHA, and the executor is a CI-held machine identity. While
...
Operational telemetry is separate from audit. Raw logs stay inside Workers observability and leave
the platform only through the configured Workers observability destinations: logs export to Axiom,
traces export to both Axiom and Sentry. Cloudflare-native Logpush and R2 log delivery are not
configured and carry no obligation while unconfigured. These destinations are the authoritative
deployed telemetry surfaces for the no-plaintext release-evidence controls: log evidence requires
the Axiom sink, trace evidence requires both providers, and a new destination cannot ship without
a registered evidence surface that fails the release gate closed until a real query adapter covers
it. Telemetry never carries free-form interpolated messages, request/response bodies, provider
...
[ADR-0085](../adr/0085-deployed-telemetry-evidence-surfaces.md),
...
provenance bound to `zaks-io/insecur/.github/workflows/cli-release.yml`, and an SBOM; the POSIX and
Windows installers fail closed unless provenance and checksum verification both pass. Customer
installation and execution continue not to require Node, npm, pnpm, Cargo, rustc, or system SQLite.
Rust dependency sources are pinned, vendored, audited, and compiled only in the controlled release
workflow; the TypeScript CLI remains the delivered behavioral reference until a
cross-implementation compatibility suite and the release gates for every supported platform permit
cutover. The pnpm controls above remain in force for the monorepo and the transition period.
...
[ADR-0086](../adr/0086-rust-cli-and-verifiable-native-distribution.md),
...
| CLI, runtime injection, and secret egress              | [0007](../adr/0007-developer-first-cli-contract.md), [0016](../adr/0016-delivery-first-secret-egress.md), [0035](../adr/0035-display-name-resolution.md), [0038](../adr/0038-protected-delivery-requires-machine-credential.md), [0052](../adr/0052-web-no-reveal-boundary-and-management-parity.md), [0086](../adr/0086-rust-cli-and-verifiable-native-distribution.md)                                                                                                                                                                                                                              |
