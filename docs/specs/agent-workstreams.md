...
| Encryption Envelope             | W3                      | Domain identity plus plaintext in, wrapped material out; decrypt only into an approved execution path, the modules on the decrypt-import allowlist decided by [ADR-0071](../adr/0071-decrypt-egress-import-boundary.md). The lint enforcement lives in `eslint.config.ts`. |
