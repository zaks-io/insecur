...
| Key management               | Instance root key bootstrap (generate, escrow, load — [runbooks/instance-root-key-bootstrap.md](runbooks/instance-root-key-bootstrap.md)); root key rotation (per-version `INSTANCE_ROOT_KEY_V{n}` Secrets Store bindings, rewrap-then-retire, per the [ADR-0028 amendment](adr/0028-instance-secrets-in-secrets-store-with-escrow.md)); Organization Data Key rotation; Project Data Key rotation; failed or interrupted rotation job; audit-export signing key bootstrap and rotation ([runbooks/audit-export-signing-key-bootstrap.md](runbooks/audit-export-signing-key-bootstrap.md) — same generate/escrow/load ceremony as the root key per ADR-0045, plus public-key publication and re-publication on rotation). | `small_group_production` |
...
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Threat model and ADR consistency      | The release matches the owning threat model and accepted decisions, or records an explicit amendment.                                                                                                                                                                                                                                                                 | [whitepaper/threat-model.md](whitepaper/threat-model.md), ADR links, design review notes.                       |
