...
| No plaintext persistence        | Insecur-controlled durable surfaces never store Sensitive Values, Provider Credentials, key material, or decrypted Sensitive Metadata.                                                                             | `pnpm test:canary` ([ADR-0069](adr/0069-no-plaintext-canary-gate.md)) for Postgres, console output, and First Value HTTP/RPC egress. R2 backups, deployed Worker logs, traces, and API analytics are checked-in external evidence requirements that block `small_group_production`; their provider query/download implementations are not yet in this repository. |
...
- Canary Sensitive Value tests prove no plaintext reaches the enumerated surfaces via
  ...
  HTTP/RPC egress with base64url allowed only at `delivery.encodedValueUtf8`). External evidence
  requirements fail closed for deployed R2 backups, Worker logs, traces, and API analytics; their
  provider query/download implementations remain outstanding.
