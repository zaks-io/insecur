...
- Production Secret Sync to GitHub and direct Cloudflare Worker secrets (and Vercel when the deferred-past-V1 adapter lands).
...
- Replace ADR-0038's Machine Identity credential requirement for Protected Environment Runtime Injection; that custody gate is additional and orthogonal.
...
This gate is storage-readiness evidence only; which modules may produce decrypt plaintext (the approved-execution-path / decrypt-egress import boundary) is governed by [ADR-0071](adr/0071-decrypt-egress-import-boundary.md), not here.
...
| Root key resident surface       | In production the root key resolves only from the Secrets Store binding on demand, never from `process.env`; the plaintext-environment fallback fails closed and no key material is held module-global (ADR-0064). | Production env-fallback refusal test, no-module-global-material test, request-scoped Keyring construction evidence.                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| Sensitive Metadata encryption   | Sensitive Metadata is encrypted under tenant-bound data keys; plaintext lookup fields are limited to Opaque Resource IDs and Display Names.                                                                        | Plaintext Metadata Allowlist conformance gate (`packages/tenant-store/test/plaintext-metadata-conformance.test.ts` for Drizzle schema; `packages/tenant-store/test/rls/plaintext-metadata-conformance.integration.test.ts` for live `information_schema` in the `postgres-integration` job) for column placement, plus encryption tests for envelope correctness.                                                                                                                                                                   |
| No plaintext persistence        | Insecur-controlled durable surfaces never store Sensitive Values, Provider Credentials, key material, or decrypted Sensitive Metadata.                                                                             | `pnpm test:canary` ([ADR-0069](adr/0069-no-plaintext-canary-gate.md)) for the structurally enumerated surfaces: every Postgres column (including operation records and audit rows), in-process console output, and serialized First Value HTTP/RPC egress. On egress, base64url is permitted only at `delivery.encodedValueUtf8`; raw/base64/hex and any other field fail. R2 backups, Queue payloads, Durable Object state, KV, traces, and analytics are pending sweep adapters; deployed worker logs are preview-layer coverage. |
...
| --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Production Secret Sync                                    | Required before decrypt or provider write.                                                                                          | App Connection, encrypted Provider Credential, Sync Execution Revalidation, Explicit Provider Lookup where required, protected delivery configuration controls, and High-Assurance Challenge plus the protected delivery configuration approval path for Protected Environment syncs. |
...
- Canary Sensitive Value tests prove no plaintext reaches the enumerated surfaces via
  `pnpm test:canary` (Postgres columns, in-process console output, and serialized First Value
  HTTP/RPC egress with base64url allowed only at `delivery.encodedValueUtf8`; other surfaces
  pending sweep adapters).
...
- Plaintext Metadata Allowlist conformance tests prove every `public` schema column is registered in `packages/tenant-store/src/db/schema/plaintext-metadata-allowlist.ts` and that the registry has no orphaned entries.
