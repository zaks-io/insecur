...
| No plaintext persistence        | Insecur-controlled durable surfaces never store Sensitive Values, Provider Credentials, key material, or decrypted Sensitive Metadata.                                                                             | Planned `pnpm test:canary` ([ADR-0069](adr/0069-no-plaintext-canary-gate.md)) for the structurally enumerated surfaces: every Postgres column, including operation records and audit rows, plus in-process console output. The script and CI task are not wired yet; R2 backups, Queue payloads, Durable Object state, KV, traces, and analytics are pending sweep adapters, and deployed worker logs are preview-layer coverage. |
...
- Canary Sensitive Value tests will prove no plaintext reaches the enumerated surfaces once
  `pnpm test:canary` is wired. Today only feature-level plaintext assertions exist; the structural
  canary gate is decided but not implemented.
