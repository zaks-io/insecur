...
- Production deploy path: `Daily Release` runs at 08:43 America/Los_Angeles and may be manually dispatched without inputs. It selects the newest successful `CI` push run on `main`, rejects divergent history, deploys and smokes that SHA in Preview, then calls reusable `Deploy Production`. Production verifies the same-run smoke artifact before mutation, runs `pnpm migrate:production`, deploys `insecur-runtime`, `insecur-api`, `insecur-web`, and `insecur-site`, verifies Sentry and live API/Web/Site identities, and syncs Linear using the full SHA. A final write-only job fast-forwards `production`; failures leave it unchanged and the next run retries the newest eligible candidate. Runtime deploy preserves the root-key Secrets Store and Hyperdrive bindings; routed deploys preserve custom domains. CI does not need Secrets Store or Workers Routes write access for routine deploys.
