...
- Production deploy path: `Daily Release` runs at 08:43 America/Los_Angeles and may be manually dispatched without inputs. It selects the newest successful `CI` push run on `main`, rejects divergent history, and syncs a scheduled Linear release keyed by the full SHA. The release advances through `Preview`, `Preview Smoke`, and `Production` as the same SHA is deployed, smoked, and promoted. Production verifies the same-run smoke artifact before mutation, runs `pnpm migrate:production`, deploys `insecur-runtime`, `insecur-api`, `insecur-web`, and `insecur-site`, and verifies Sentry and live API/Web/Site identities. Linear reaches `Released` only after those checks; then a final write-only job fast-forwards `production`. Failures leave the branch unchanged and the next run retries the newest eligible candidate, including Linear completion during branch reconciliation. Runtime deploy preserves the root-key Secrets Store and Hyperdrive bindings; routed deploys preserve custom domains. CI does not need Secrets Store or Workers Routes write access for routine deploys.
- Deploy GitHub Environment variables: Preview uses `PREVIEW_INSTANCE_ID`, `PREVIEW_HYPERDRIVE_ID`, `PREVIEW_RUNTIME_ROOT_KEY_STORE_ID`, `PREVIEW_RUNTIME_ROOT_KEY_SECRET_NAME`, `PREVIEW_RUNTIME_BACKUPS_BUCKET_NAME`, and `PREVIEW_API_RATELIMIT_*_NAMESPACE_ID`; Production uses `PRODUCTION_INSTANCE_ID`, `PRODUCTION_HYPERDRIVE_ID`, `PRODUCTION_RUNTIME_ROOT_KEY_STORE_ID`, `PRODUCTION_RUNTIME_ROOT_KEY_SECRET_NAME`, `PRODUCTION_RUNTIME_BACKUPS_BUCKET_NAME`, and `PRODUCTION_API_RATELIMIT_*_NAMESPACE_ID`. Both environments use `WORKOS_CLIENT_ID` and `WORKOS_AUTHKIT_ORIGIN` (the per-environment hosted WorkOS AuthKit authorization origin threaded into the Web BFF CSP `form-action` directive, INS-417). `SENTRY_AUTH_TOKEN` is required for release/source-map upload and is read via `secrets.SENTRY_AUTH_TOKEN` (repository secret by default; optional per-environment override — see `docs/build-tooling.md` § Sentry auth token setup). Production release sync requires the repository secret `LINEAR_ACCESS_KEY`; CLI release sync requires the separate repository secret `CLI_LINEAR_ACCESS_KEY`. The workflows map environment-scoped names into package-level `INSECUR_*` variables consumed by `scripts/wrangler-deploy-config.mjs` and set `INSECUR_REQUIRE_SENTRY_SOURCEMAPS=true` during real deploys.
