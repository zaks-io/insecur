...
- Preview checks: no per-PR preview workflow; DB/runtime PR database validation is `CI` → `Verify` using Docker Compose Postgres. Local Web preview is `pnpm --filter @insecur/web deploy:preview`: it builds local code, derives deploy identity, and runs the Web package's Wrangler deploy with existing Cloudflare-side variables preserved. The daily release train calls reusable `Deploy Preview` and `Preview Smoke` stages for one exact CI-green `main` SHA; neither stage is independently dispatchable. Preview URLs are `https://api.preview.insecur.cloud`, `https://app.preview.insecur.cloud`, and `https://preview.insecur.cloud`; Runtime has no public route. API/Web/Site `/healthz` responses include `service`, `deploySha`, `runId`, and `deployedAt`, and preview smoke fails if any deploy SHA differs from the candidate.
- Production deploy path: `Daily Release` runs at 08:43 America/Los_Angeles and may be manually dispatched without inputs. It selects the newest successful `CI` push run on `main` and rejects divergent history. The same SHA is deployed to Preview and smoked before a terminal release is recorded in the `preview.insecur.dev` Linear pipeline. Production then verifies the same-run smoke artifact before mutation, runs `pnpm migrate:production`, deploys `insecur-runtime`, `insecur-api`, `insecur-web`, and `insecur-site`, and verifies Sentry and live API/Web/Site identities before recording the terminal `insecur.dev` Linear release. A final write-only job then fast-forwards `production`. Failures leave the branch unchanged and the next run retries the newest eligible candidate, including Production Linear sync during branch reconciliation. Runtime deploy preserves the root-key Secrets Store and Hyperdrive bindings; routed deploys preserve custom domains. CI does not need Secrets Store or Workers Routes write access for routine deploys.
- Deploy GitHub Environment variables: Preview uses `PREVIEW_INSTANCE_ID`, `PREVIEW_HYPERDRIVE_ID`, `PREVIEW_RUNTIME_ROOT_KEY_STORE_ID`, `PREVIEW_RUNTIME_ROOT_KEY_SECRET_NAME`, `PREVIEW_RUNTIME_BACKUPS_BUCKET_NAME`, and `PREVIEW_API_RATELIMIT_*_NAMESPACE_ID`; Production uses `PRODUCTION_INSTANCE_ID`, `PRODUCTION_HYPERDRIVE_ID`, `PRODUCTION_RUNTIME_ROOT_KEY_STORE_ID`, `PRODUCTION_RUNTIME_ROOT_KEY_SECRET_NAME`, `PRODUCTION_RUNTIME_BACKUPS_BUCKET_NAME`, and `PRODUCTION_API_RATELIMIT_*_NAMESPACE_ID`. Both environments use `WORKOS_CLIENT_ID` and `WORKOS_AUTHKIT_ORIGIN` (the per-environment hosted WorkOS AuthKit authorization origin threaded into the Web BFF CSP `form-action` directive, INS-417). Each environment owns a pipeline-scoped `LINEAR_ACCESS_KEY`: Preview targets `preview.insecur.dev`; Production targets `insecur.dev`. `SENTRY_AUTH_TOKEN` is required for release/source-map upload and is read via `secrets.SENTRY_AUTH_TOKEN` (repository secret by default; optional per-environment override — see `docs/build-tooling.md` § Sentry auth token setup). The `cli-release` job uses the Production environment, where its isolated `CLI_LINEAR_ACCESS_KEY` is available. The workflows map environment-scoped names into package-level `INSECUR_*` variables consumed by `scripts/wrangler-deploy-config.mjs` and set `INSECUR_REQUIRE_SENTRY_SOURCEMAPS=true` during real deploys.
...
- Code-host branch/ruleset enforcement: classic GitHub branch protection API returned 404. Active repository ruleset `Production` targets the default branch and enforces deletion protection, non-fast-forward protection, linear history, PR thread resolution, squash-only merge, and required status check `Verify`. Active ruleset `Production release ledger` targets `production`, blocks deletion and non-fast-forward updates, and requires the GitHub Actions-owned `Production release verified` status written only by the release finalizer.
...
- Default-branch baseline health: refresh live with `gh run list --workflow CI --branch main --limit 1` and `gh run list --workflow 'Daily Release' --branch main --limit 1`; compare the latest successful release SHA with the `production` branch and public `/healthz` identities
...
- Preview/production: scheduled or input-free manual `Daily Release`; local Web code deploy: `pnpm --filter @insecur/web deploy:preview`
