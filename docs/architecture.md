...
The goal is not to clone a full enterprise secrets platform. The first-class use case is developers and small groups using coding agents across many projects who want sensitive development values out of local secret files first, then professional production controls for a focused Cloudflare, Vercel, and GitHub Actions stack.

V1 centers on one product spine: insecur stores the canonical secret versions, injects development values just in time for local commands through the CLI so agents do not need `.env` files, and later syncs derived values to Cloudflare, Vercel, and GitHub when native provider secrets are useful. Agents should be able to cause approved secret use without steady-state plaintext secret files or Secret Reveal.

The first V1 promise is Diskless Development Secret Use: stop giving coding agents plaintext local secret files while preserving fast local development. The production promise remains stronger and later: let agents and CI cause approved deploy and runtime workflows without giving local agents or ordinary human sessions a read path to Protected Environment Sensitive Values.

Diskless development only works as a real safety improvement if the CLI does not replace `.env` sprawl with "inject everything in this Environment." A development Environment may hold many secrets for different local tasks, but each Runtime Injection Policy is workflow-scoped and injects only its exact bound values. That lets a developer switch between dev server, test suite, migration, and preview workflows without giving each child process every secret the project knows about.

The logical key-value store is Environment-scoped: each Secret Shape defines an env-var-safe Variable Key such as `DATABASE_URL`, and each Environment has at most one Secret value for that shape. V1 Variable Keys match `^[A-Z_][A-Z0-9_]*$` so Runtime Injection, GitHub Actions, Vercel, and shell environments stay predictable. Runtime Injection turns selected Environment Secrets into child-process environment variables using those Variable Keys. Secret Sync may map a selected Secret to a provider-side destination name, defaulting to the Variable Key unless an exact binding says otherwise.
...
Hosted onboarding should make the first usable workspace appear without naming ceremony. When Public Onboarding admits a user, Guided Organization Provisioning creates a Personal Organization, Default Team, owner Membership, first Project, and non-protected development Environment with default Display Names. It should also create readable default CLI Profile Slugs and Runtime Injection Policy names for the starter local workflow. Those defaults are editable suggestions, while workflow-facing selectors remain scoped-unique so `--profile` and `--policy-name` stay unambiguous. The Personal Organization is still a normal Organization: it can be renamed, extended, and grown through Invitations and Memberships without introducing a separate single-user product mode.

The first product-value moment should be provider-free and should use the real product commands. A First Value Proof uses service-side generation through `secrets set`, a Blind Secret Write, and explicit one-command secret selection through `run --variable-key` in the starter development Environment to show a secret can be used without putting the Sensitive Value in command output, local secret files, provider setup, or an agent transcript. This proves Diskless Development Secret Use and delivery-without-reveal in caller output, not that arbitrary local child processes or active malware cannot read injected development values after the Runtime Trust Boundary.
...
V1 should expose Delivery Risk Policy as presets rather than a custom policy editor. Guided Organization Provisioning applies Balanced automatically and does not show a preset picker during first onboarding. Strict, Balanced, and Automation-Friendly are simple user-facing choices backed by versioned policy templates, scoped effective policy, migration behavior, and audit records so later enterprise policy surfaces can build on the same infrastructure. Selecting Automation-Friendly after onboarding is a Risk-Broadening Delivery Change.
...
- MFA is required for human users before v1 production use. WorkOS AuthKit owns the human authentication path, with no SMS factor. High-Assurance Challenge clearing currently requires a fresh passkey assertion; enrolled TOTP metadata alone never satisfies that gate.
- High-Assurance Challenges are required for Sensitive Detail Gate, Protected Environment Approval Request approval/rejection, Promotion/rollback, Protected Approval Policy changes, protected Runtime Injection Policy changes, App Connection create/reauthorization/scope changes, Protected Environment Secret Sync create/enable/manual run, protected Secret Sync Binding changes, repository-scoped GitHub overrides, protected Shared Secret Source attachment, Push Device Registration creation/replacement, and mutating Service Access controls. Requester cancellation of a pending Approval Request does not require a High-Assurance Challenge because it only closes the request without delivery changes.
...
- Durable resource selectors use opaque resource IDs, not plaintext names or slugs. Variable Keys are the application-facing keys for environment delivery, not resource identity selectors.
...
- Sensitive Values are accepted only through safe sensitive input paths: request bodies over TLS, CLI stdin, masked prompts for human TTY callers, service generation, provider authorization flows, or development-only Secret Import. Managed Secret values are valid UTF-8 text in V1; invalid UTF-8 fails with `secret.invalid_encoding`, with no binary mode, implicit base64 decoding, or replacement-character decoding. Managed Secret value size is measured in encoded UTF-8 bytes after validation, not characters, and values over 64 KiB fail with `secret.value_too_large` before write. The 64 KiB storage limit is not a provider compatibility guarantee; Secret Sync adapters enforce destination-specific Provider Value Size Limits and fail the whole write set with `sync.provider_value_too_large` before provider write when any binding is too large. Secret Sync must not compress, truncate, chunk, implicitly encode, or otherwise transform values to fit provider caps. CLI stdin preserves the decoded value exactly, including trailing newlines and multiline content, with no trimming or normalization. Zero-length values are valid only when explicitly allowed and otherwise fail with `secret.empty_value`. Non-interactive secret writes must explicitly select service generation or stdin input and fail with `secret.input_required` otherwise. Values are never accepted in URLs, query strings, route params, CLI arguments, named local value files for ordinary secret writes, or GET requests.
...
- `insecur run <profile-slug-or-id> -- <command>` and future `insecur sync` use `.insecur.json` defaults unless flags override them.
...
- Ergonomic CLI selectors such as `--policy-name` and `--profile` resolve scoped-unique workflow labels within a fully resolved parent scope, then act on opaque IDs. Human and JSON output should echo resolved target type, Display Name or slug, opaque ID, and parent scope so humans and agents can read what happened at a glance.
...
- Protected Environment secrets do not support Secret Reveal and have no reveal UI/API/CLI/Service Access path.
...
- A Runtime Injection Policy is workflow-scoped: an Environment may hold unrelated secrets for other workflows, but the selected policy injects only the exact bound secret set for its command.
...
The audit trail for a sync operation should include lease claim, Sync Execution Revalidation result, provider write summaries, retry, completion or `incomplete` or cancellation, and lease release events.
