...
Balanced allows non-protected development automation by default, but not blanket preview automation. Each non-protected preview Environment needs its own Preview Automation Opt-In before Agent-Reachable Channels can perform configured delivery actions for that environment. Preview Automation Opt-In grants execution-only Preview Automation Authority: agents may run existing Runtime Injection Policies and existing Secret Syncs/Secret Sync Bindings, but may not create or change provider connections, connection boundaries, sync targets, runtime policies, provider targets, or the delivered Secret set.
