...
The current GitHub OAuth allowlist is scaffold-only and should be replaced before public multi-tenant use. If GitHub identity remains useful, it should be routed through WorkOS rather than maintained as a separate app-owned OAuth stack. WorkOS API credentials become managed deployment secrets, while provider integrations remain modeled as app connections.
