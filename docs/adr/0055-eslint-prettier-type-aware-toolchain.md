...
- **Leave function size and file length to code review.** Rejected: CodeRabbit and human review are advisory and non-deterministic, so a sprawling function lands whenever a reviewer does not flag it. A linted budget in the required `verify` check fails closed and applies to every author identically.
