...
This model is a production delivery prerequisite. Secret Sync and Runtime Injection must fail closed until the Storage Security Gate verifies root key placement, organization and project data keys, key versions, provider credentials and Sensitive Metadata encrypted under tenant-bound data keys, and authenticated-data binding for encrypted secret, provider credential, and Sensitive Metadata records.
