...
- The Public Site deploys through the same shared Worker fleet workflows as the other app Workers. Preview deploys route `preview.insecur.cloud` to `insecur-site-preview`; production deploys route `insecur.cloud` and `www.insecur.cloud` to `insecur-site` as part of the manual `Deploy Production` workflow. Do not auto-publish any production Worker on every `main` push until legal and security publication gates are settled. This ADR owns only the Public Site Worker boundary; the fleet workflow owns deploy orchestration.
