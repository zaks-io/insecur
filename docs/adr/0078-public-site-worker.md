...
- The Public Site deploys through the same shared Worker fleet workflows as the other app Workers. Preview deploys route `preview.insecur.cloud` to `insecur-site-preview`; production deploys route `insecur.cloud` and `www.insecur.cloud` to `insecur-site` as part of the CI-gated `Deploy Production` workflow after successful `CI` on `main`. This ADR owns only the Public Site Worker boundary; the fleet workflow owns deploy orchestration.
