...
- The Public Site deploys through the same shared Worker fleet workflows as the other app Workers. Preview deploys route `preview.insecur.cloud` to `insecur-site-preview`; production deploys route `insecur.cloud` and `www.insecur.cloud` to `insecur-site` as part of `Deploy Production` after exact-SHA CI and deployed Preview Smoke evidence. This ADR owns only the Public Site Worker boundary; the fleet workflow owns deploy orchestration.
