...
insecur hardens its dependency install path with pnpm 10. Lifecycle scripts are blocked by default and gated by an explicit `onlyBuiltDependencies` allowlist (for example `esbuild` and `workerd`), with `strictDepBuilds: true` so an install fails loudly when a dependency wants to run a build or postinstall script that has not been decided. Newly published versions are quarantined with `minimumReleaseAge: 4320` (minutes, three days) in `pnpm-workspace.yaml`, so a version cannot be installed until it has been public long enough for a compromised release to surface. Renovate honors the same floor via `minimumReleaseAge: 3 days` and `internalChecksFilter: strict` in `renovate.json`, so it creates neither a branch nor a pull request before the floor elapses. Renovate tests eligible updates on hidden `renovate/**` branches and creates a pull request only after CI succeeds; routine non-major updates then automerge through the protected-branch gates, vulnerability fixes are grouped into one green-only pull request, and major updates require Dependency Dashboard approval. Automatic Renovate config-migration pull requests are disabled. Install-time script blocking and release-age quarantine live in `pnpm-workspace.yaml`; update-delivery policy lives in `renovate.json` and is enforced by `pnpm conformance:renovate`. Because `minimumReleaseAge` and `strictDepBuilds` are pnpm 10 features and the repository is pinned at `pnpm@9.0.0`, this decision forces the pnpm 9 to 10 upgrade. It implements and extends the dependency and supply-chain scanning posture that ADR-0008 requires.
