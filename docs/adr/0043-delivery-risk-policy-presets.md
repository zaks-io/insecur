...
Preview Automation Authority is execution authority only, whether granted by Preview Automation Opt-In under Balanced or by Automation-Friendly by default. Agents may run already-configured Runtime Injection Policies and already-configured Secret Syncs with exact existing Secret Sync Bindings for the preview Environment where authority applies. They may not create or change App Connections, Connection Boundaries, Secret Syncs, Secret Sync Bindings, Runtime Injection Policies, provider targets, or the delivered Secret set without a separate Risk-Broadening Delivery Change.
