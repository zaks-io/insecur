...
Some audit fields are Sensitive Metadata even though they are not plaintext secrets. Historical secret/display names, provider target names, and policy binding names must be encrypted at rest under tenant-bound data keys. They may appear in full-fidelity exports for authorized security review. Low-privilege exports must use immutable IDs and hashes and exclude Sensitive Metadata that reveals security-relevant structure.
