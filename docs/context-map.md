...
| `@insecur/local-store`                                            | Local Mode machine root key custody   | `KeyStore` seam, OS keychain shell-out adapters, `0600` file fallback, metadata-only weaker-posture notice                                                                         | Encrypted local store, CLI wiring, passphrase/daemon unlock, tenant key hierarchy                                                              |
