...
| `@insecur/machine-auth` (staged, M4; no exchange route wired yet) | Machine Identity auth method exchange                         | GitHub Actions OIDC federation exchange, short-lived machine access token mint/verify, trusted-source matching, exchange audit events                                                                 | Human authentication, Machine Identity membership persistence, Effective Access expansion, HTTP route/API composition, Environment Deploy Keys |
...
| `@insecur/local-store`                                            | Local Mode machine root key custody and encrypted local store | `KeyStore` seam, OS keychain shell-out adapters, `0600` file fallback, SQLite local Secret Version Store / Injection Grant / audit seams (current versions only), metadata-only weaker-posture notice | CLI wiring, passphrase/daemon unlock, version history, backup/export                                                                           |
