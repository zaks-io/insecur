...
| `@insecur/machine-auth`            | Machine Identity auth method exchange | GitHub Actions OIDC federation exchange, short-lived machine access token mint/verify, trusted-source matching, exchange audit events                                              | Human authentication, Machine Identity membership persistence, Effective Access expansion, HTTP route/API composition, Environment Deploy Keys |
