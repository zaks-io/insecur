...
| `storage.no_plaintext`       | Insecur-controlled durable surfaces never store Sensitive Values, Provider Credentials, or key material.                                                                                                                                                                                                                                                                        | `pnpm test:canary` for enumerated surfaces plus fail-closed external evidence requirements for R2 backups, deployed Worker logs, traces, and API analytics ([ADR-0069](adr/0069-no-plaintext-canary-gate.md)); external sweep implementations remain required.               |
...
| `telemetry.safe`             | Logs, telemetry, traces, analytics, and error output are secret-free and metadata-only by default.                                                                                                                                                                                                                                                                              | Telemetry allowlist review; `pnpm test:canary` for enumerated surfaces plus required external zero-finding evidence for deployed logs, traces, and API analytics ([ADR-0069](adr/0069-no-plaintext-canary-gate.md)); the external query implementations are not yet present. |
