...
- Staged Change Set or batch publish UI.
...
- Self-hosting.
- Secret Reveal or plaintext export for Protected Environment secrets.
...
| `deploy.production`          | Production Worker deploy runs through approved CI/CD and production environment approval.                      | CI run IDs, GitHub Environment approval, production smoke results.                                   |
...
| `tenant.isolation`           | Organization/project membership, Effective Access, organization-qualified routes, and denial shape hold.       | Cross-tenant authorization tests and route inventory.                                                |
| `tenant.rls`                 | Tenant-Scoped Store uses real Postgres RLS as the `NOBYPASSRLS` runtime role, with no raw executor escape.     | Neon branch RLS tests, migration review, store tests, runtime-role guard.                            |
...
| `storage.no_plaintext`       | Insecur-controlled durable surfaces never store Sensitive Values, Provider Credentials, or key material.       | Canary-value scans across Postgres, R2, logs, caches, operation records, audit, traces, and backups. |
...
| `audit.integrity`            | Tenant-qualified audit events exist for allowed and denied actions, and audit export verification passes.      | Audit tests, `insecur audit verify`, hash-chain and manifest evidence.                               |
| `backup.restore`             | Encrypted backup and one full restore drill can decrypt a recovery canary in a fresh environment.              | Restore runbook drill, measured restore report, canary verification.                                 |
| `runbook.catalog`            | Required security runbooks exist, have dry-run/execute/verify sections, and critical drills have evidence.     | Runbook review and drill IDs.                                                                        |
...
| `telemetry.safe`             | Logs, telemetry, traces, analytics, and error output are secret-free and metadata-only by default.             | Telemetry allowlist review, canary tests, log inspection.                                            |
...
hash chain and HMACed manifest.
