...
| `storage.no_plaintext`       | Insecur-controlled durable surfaces never store Sensitive Values, Provider Credentials, or key material.                                                                                                                                                                                                                                                                        | Planned `pnpm test:canary` for the enumerated surfaces (Postgres columns, operation records, audit rows, in-process console output) plus registered sweep adapters for R2, caches, traces, and backups ([ADR-0069](adr/0069-no-plaintext-canary-gate.md)). The script and CI task are decided but not wired yet.                      |
...
| `telemetry.safe`             | Logs, telemetry, traces, analytics, and error output are secret-free and metadata-only by default.                                                                                                                                                                                                                                                                              | Telemetry allowlist review; planned `pnpm test:canary` for the enumerated surfaces (Postgres columns, operation records, audit rows, in-process console output) plus registered sweep adapters for the rest ([ADR-0069](adr/0069-no-plaintext-canary-gate.md)), log inspection. The script and CI task are decided but not wired yet. |
