...
`pnpm dev:db:reset && pnpm test:rls && pnpm test:e2e` runs the forced-RLS tenant suite and the
First Value loop locally, and CI's `postgres-integration` job enforces the same layer with
