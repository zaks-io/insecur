...
- The machine-local root key lives in the OS credential store via shell-outs to OS tooling behind one `KeyStore` seam. Local Mode fails closed when no supported credential store is available. `INSECUR_ALLOW_INSECURE_FILE_KEYSTORE=1` explicitly enables a development-only `0600` key file beside the ciphertext and therefore gives up backup separation ([ADR-0080](adr/0080-local-mode-accountless-development-custody.md)).
...
1. **Create missing remote resources** by replaying client-minted IDs from local project and environment metadata. Secret writes send the version-conditional `ifCurrentVersionAbsent` guard on the Blind Secret Write: the server rejects the write with `import.existing_secret` when a Current Version already exists at write time (enforced atomically inside the write, closing the check-then-write race) while completing a Secret Shape that exists without a Current Version. The conflict resolves through the possession check into convergence (same value) or `migrate.remote_diverged` (different value); migrate never supersedes a remote value.
...
idempotency conflict, operation wait) must ship remediation content. The **Remediation** column marks
`required` when an emitted error must include a `remediation` object; `-` means not required for
that code. Enforcement mirrors the exit/HTTP lockstep style (ADR-0062).
...
a resource-existence oracle, even where their exit codes differ. That rule governs cross-boundary
denials; `runtime_policy.secret_binding_environment_mismatch` → `400` is a deliberate intra-tenant
carve-out (the actor is already authorized for the scope; see
[ADR-0062](adr/0062-package-seam-failures-are-errorbody-compatible.md) as amended), not an accidental
invariant violation.

| Stable error code                                    | Exit | HTTP                | Remediation | Notes                                                                                                                                                                                                                                                                                        |
| ---------------------------------------------------- | ---- | ------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `auth.required`                                      | `3`  | `401`               | `required`  | Session expired or absent.                                                                                                                                                                                                                                                                   |
| `auth.config_invalid`                                | `1`  | `503`               | `-`         | Auth Worker bindings are missing or invalid; names the configuration class only.                                                                                                                                                                                                             |
| `auth.expired`                                       | `3`  | `401`               | `required`  | Session expired.                                                                                                                                                                                                                                                                             |
| `auth.invalid`                                       | `3`  | `401`               | `required`  | Session invalid.                                                                                                                                                                                                                                                                             |
| `auth.reauth_required`                               | `3`  | `401`               | `required`  | Fresh re-authentication required.                                                                                                                                                                                                                                                            |
...
| `auth.deploy_key_invalid`                            | `3`  | `401`               | `-`         | Environment Deploy Key secret failed verification or runtime policy key is not allowlisted.                                                                                                                                                                                                  |
| `auth.deploy_key_disabled`                           | `4`  | `403`               | `-`         | Environment Deploy Key is disabled.                                                                                                                                                                                                                                                          |
| `auth.deploy_key_wrong_environment`                  | `4`  | `403`               | `-`         | Environment Deploy Key is not authorized for the requested project and environment.                                                                                                                                                                                                          |
| `auth.deploy_key_overbroad_scope`                    | `4`  | `403`               | `-`         | Environment Deploy Key credential scopes exceed the Runtime Injection bundle.                                                                                                                                                                                                                |
...
| `validation.request_body_too_large`                  | `2`  | `413`               | `-`         | Public API request body exceeds the bounded JSON envelope limit and is rejected before analytics, abuse accounting, authentication, RPC, or persistence.                                                                                                                                     |
...
| `validation.invalid_profile_slug`                    | `2`  | `n/a (client-side)` | `-`         | CLI Profile Slug must be lower-kebab.                                                                                                                                                                                                                                                        |
| `cli.profile_not_found`                              | `5`  | `n/a (client-side)` | `-`         | CLI Profile slug or opaque ID does not exist in local user config.                                                                                                                                                                                                                           |
| `cli.profile_slug_in_use`                            | `6`  | `n/a (client-side)` | `-`         | CLI Profile Slug already exists in local user config.                                                                                                                                                                                                                                        |
| `cli.display_name_not_found`                         | `5`  | `n/a (client-side)` | `-`         | Display Name Resolution found zero exact matches in the scoped list (ADR-0035).                                                                                                                                                                                                              |
| `cli.display_name_ambiguous`                         | `2`  | `n/a (client-side)` | `-`         | Display Name Resolution found two or more exact matches in the scoped list (ADR-0035).                                                                                                                                                                                                       |
...
| `environment.non_protected_environment`              | `4`  | `403`               | `-`         | Protected draft secret writes are blocked for non-Protected Environments.                                                                                                                                                                                                                    |
...
| `store.unavailable`                                  | `8`  | `503`               | `-`         | Transient database connection-layer failure (Hyperdrive pool exhaustion, server shutdown, connection loss); safe to retry after a short delay.                                                                                                                                               |
| `storage.gate_blocked`                               | `1`  | `503`               | `-`         | Storage Security Gate blocked production delivery before decrypt or provider write.                                                                                                                                                                                                          |
| `storage.gate_unknown`                               | `1`  | `503`               | `-`         | Storage Security Gate readiness evidence is missing or unreachable for production delivery.                                                                                                                                                                                                  |
| `crypto.decrypt_failed`                              | `1`  | `500`               | `-`         | Opaque single failure; HTTP `500` must not differentiate by cause ([ADR-0062](https://github.com/zaks-io/insecur/blob/main/docs/adr/0062-package-seam-failures-are-errorbody-compatible.md) amendment).                                                                                      |
...
| `audit.export_entry_limit_exceeded`                  | `2`  | `400`               | `-`         | Audit export time range matches more entries than a single export bundle allows; narrow the range and retry.                                                                                                                                                                                 |
| `high_assurance.evidence_missing`                    | `10` | `401`               | `-`         | Cleared challenge evidence is absent; fail closed to fresh step-up (ADR-0032).                                                                                                                                                                                                               |
| `high_assurance.evidence_expired`                    | `10` | `401`               | `-`         | Challenge evidence validity window elapsed; fresh step-up required (ADR-0032).                                                                                                                                                                                                               |
| `high_assurance.operation_mismatch`                  | `5`  | `404`               | `-`         | Challenge evidence is not bound to the requested operation.                                                                                                                                                                                                                                  |
| `high_assurance.actor_mismatch`                      | `4`  | `403`               | `-`         | Clearing user is not authorized for this bounded operation.                                                                                                                                                                                                                                  |
| `high_assurance.already_consumed`                    | `10` | `401`               | `-`         | Single-use challenge evidence was already consumed.                                                                                                                                                                                                                                          |
| `high_assurance.clearing_denied`                     | `4`  | `403`               | `-`         | Clearing user lacks scope or operation state rejects clearing.                                                                                                                                                                                                                               |
| `high_assurance.session_assurance_failed`            | `3`  | `401`               | `-`         | WorkOS session assurance did not satisfy fresh high-assurance factor posture (INS-49).                                                                                                                                                                                                       |
| `high_assurance.invalid_risk_reason`                 | `2`  | `400`               | `-`         | Unknown or malformed high-assurance risk reason code.                                                                                                                                                                                                                                        |
...
| `import.existing_secret`                             | `6`  | `409`               | `-`         | Existing-secret conflict in the target Environment (preflight, create-only, or version-conditional write).                                                                                                                                                                                   |
...
| `backup_restore.not_armed`                           | `1`  | `503`               | `-`         | Restore entrypoint called while the RESTORE_DB arming binding is absent (ADR-0084 fail-closed default).                                                                                                                                                                                      |
| `backup_restore.target_is_live`                      | `1`  | `503`               | `-`         | RESTORE_DB points at the normal database target; restore refuses before any statement runs (ADR-0084).                                                                                                                                                                                       |
| `backup_restore.target_not_fresh`                    | `6`  | `409`               | `-`         | Restore target failed the fresh-target proof (journal marker, instance identity, or organizations already present). Discard the target and retry on a fresh one.                                                                                                                             |
| `backup_restore.import_conflict`                     | `6`  | `409`               | `-`         | Another restore import holds the target's exclusion lock or journal marker (ADR-0084 at-most-one import per target).                                                                                                                                                                         |
| `backup_restore.artifact_not_found`                  | `5`  | `404`               | `-`         | No sealed scheduled-export artifact exists at the named reference.                                                                                                                                                                                                                           |
| `backup_restore.artifact_invalid`                    | `2`  | `400`               | `-`         | Sealed artifact is malformed or failed authenticated decryption under the AAD-bound instance ID.                                                                                                                                                                                             |
| `backup_restore.header_mismatch`                     | `2`  | `400`               | `-`         | Advisory header pre-check failed fast (format marker, instance, or root key version); trust still derives from the AEAD open.                                                                                                                                                                |
| `backup_restore.export_operation_mismatch`           | `6`  | `409`               | `-`         | Export success evidence is missing, hash-mismatched, or does not link the artifact to its source export Operation.                                                                                                                                                                           |
...
| `runtime_policy.invalid_bindings`                    | `2`  | `400`               | `-`         | Policy bindings are missing, duplicate, or not exact opaque IDs / Variable Keys.                                                                                                                                                                                                             |
| `runtime_policy.pattern_binding_rejected`            | `2`  | `400`               | `-`         | Wildcard, prefix, suffix, regex, tag, folder, or other broad secret selection is rejected.                                                                                                                                                                                                   |
| `runtime_policy.secret_binding_not_found`            | `5`  | `404`               | `-`         | Bound secret ID does not exist in the policy's organization and project scope.                                                                                                                                                                                                               |
| `runtime_policy.secret_binding_environment_mismatch` | `2`  | `400`               | `-`         | Bound secret ID exists in the organization but belongs to a different Environment than the policy. `400` (not `404`) is a deliberate intra-tenant carve-out per ADR-0062: the configurer is already scope-authorized, so distinguishing wrong-env from not-found is not an existence oracle. |
| `runtime_policy.version_immutable`                   | `6`  | `409`               | `-`         | Used Runtime Injection Policy Versions cannot be mutated.                                                                                                                                                                                                                                    |
| `runtime_policy.protected_use_blocked`               | `4`  | `403`               | `-`         | Protected Environment policy use blocked until machine custody and Storage Security Gate readiness are satisfied.                                                                                                                                                                            |
| `runtime_policy.disabled`                            | `4`  | `403`               | `-`         | Runtime Injection Policy is disabled.                                                                                                                                                                                                                                                        |
...
| `approval.request_not_found`                         | `5`  | `404`               | `-`         | Approval Request not found for the tenant-qualified ID.                                                                                                                                                                                                                                      |
| `approval.request_not_pending`                       | `6`  | `409`               | `-`         | Approval Request is not pending or lost a compare-and-set race.                                                                                                                                                                                                                              |
| `approval.invalid_draft_selection`                   | `2`  | `400`               | `-`         | Promote body lists Draft Version IDs that are missing, not draft, or not in the target Environment.                                                                                                                                                                                          |
| `approval.wildcard_selection_rejected`               | `2`  | `400`               | `-`         | Wildcard or all-staged draft selection tokens are rejected at the API edge.                                                                                                                                                                                                                  |
| `approval.promotion_change_set_immutable`            | `6`  | `409`               | `-`         | Promotion Change Set is immutable after the Approval Request is created.                                                                                                                                                                                                                     |
| `approval.rollback_target_not_eligible`              | `2`  | `400`               | `-`         | Rollback target version is not a retained published version eligible for copy.                                                                                                                                                                                                               |
| `approval.draft_version_not_discardable`             | `2`  | `400`               | `-`         | Draft Version Discard target is not found, or is not currently a draft (already promoted or discarded elsewhere).                                                                                                                                                                            |
...
| `connection.not_found`                               | `5`  | `404`               | `-`         | App Connection not found for the tenant-qualified ID.                                                                                                                                                                                                                                        |
| `connection.resource_conflict`                       | `6`  | `409`               | `-`         | App Connection ID or unique setup constraint already exists.                                                                                                                                                                                                                                 |
| `connection.disconnected`                            | `7`  | `n/a (client-side)` | `-`         | App Connection is disconnected; sync execution fails closed until reconnected.                                                                                                                                                                                                               |
| `connection.reauthorization_required`                | `7`  | `n/a (client-side)` | `-`         | App Connection requires provider reauthorization before sync execution.                                                                                                                                                                                                                      |
| `connection.pending_setup`                           | `7`  | `n/a (client-side)` | `-`         | App Connection setup is incomplete; sync execution fails closed.                                                                                                                                                                                                                             |
| `connection.provider_registration_missing`           | `7`  | `n/a (client-side)` | `-`         | Required Provider App Registration is missing for the Instance and connection method.                                                                                                                                                                                                        |
| `connection.credential_missing`                      | `7`  | `n/a (client-side)` | `-`         | Credential-backed App Connection lacks an active encrypted provider credential.                                                                                                                                                                                                              |
| `connection.validation_failed`                       | `7`  | `n/a (client-side)` | `-`         | Cloudflare scoped-token validation failed; connection remains non-writeable until current evidence succeeds.                                                                                                                                                                                 |
| `connection.boundary_mismatch`                       | `2`  | `n/a (client-side)` | `-`         | Cloudflare token scope does not match the pinned account or Worker script boundary.                                                                                                                                                                                                          |
| `connection.invalid_connection_method`               | `2`  | `400`               | `-`         | App Connection operation targets an unsupported provider or connection method.                                                                                                                                                                                                               |
| `sync.not_found`                                     | `5`  | `404`               | `-`         | Secret Sync not found for the tenant-qualified ID.                                                                                                                                                                                                                                           |
| `sync.disabled`                                      | `4`  | `403`               | `-`         | Secret Sync is disabled and cannot execute.                                                                                                                                                                                                                                                  |
| `sync.invalid_bindings`                              | `2`  | `400`               | `-`         | Secret Sync binding set failed validation.                                                                                                                                                                                                                                                   |
| `sync.pattern_binding_rejected`                      | `2`  | `400`               | `-`         | Pattern, wildcard, or inventory-based bindings are rejected.                                                                                                                                                                                                                                 |
| `sync.secret_binding_not_found`                      | `5`  | `404`               | `-`         | Bound secret not found in the source Environment.                                                                                                                                                                                                                                            |
| `sync.secret_binding_environment_mismatch`           | `2`  | `400`               | `-`         | Bound secret belongs to a different Environment.                                                                                                                                                                                                                                             |
| `sync.invalid_destination`                           | `2`  | `400`               | `-`         | Provider destination or target configuration is invalid.                                                                                                                                                                                                                                     |
| `sync.resource_conflict`                             | `6`  | `409`               | `-`         | Secret Sync ID or unique setup constraint already exists.                                                                                                                                                                                                                                    |
| `sync.connection_not_eligible`                       | `7`  | `n/a (client-side)` | `-`         | App Connection is not eligible for this Secret Sync.                                                                                                                                                                                                                                         |
| `sync.display_name_in_use`                           | `6`  | `409`               | `-`         | Display name already used by another Secret Sync in scope.                                                                                                                                                                                                                                   |
| `notification.invalid_event_code`                    | `2`  | `400`               | `-`         | Webhook subscription event type is missing or not selectable.                                                                                                                                                                                                                                |
| `notification.subscription_not_found`                | `5`  | `404`               | `-`         | Webhook subscription not found for the tenant-qualified ID.                                                                                                                                                                                                                                  |
| `notification.delivery_failed`                       | `1`  | `500`               | `-`         | Event notification delivery failed for a subscribed webhook.                                                                                                                                                                                                                                 |
| `notification.signing_secret_missing`                | `1`  | `500`               | `-`         | Active webhook signing secret is missing for delivery or rotation.                                                                                                                                                                                                                           |
| `provider_app_registration.not_found`                | `5`  | `404`               | `-`         | Provider App Registration not found for the Instance and connection method.                                                                                                                                                                                                                  |
| `provider_app_registration.not_configured`           | `7`  | `n/a (client-side)` | `-`         | Provider App Registration exists but instance setup is incomplete.                                                                                                                                                                                                                           |
| `provider_app_registration.already_exists`           | `6`  | `409`               | `-`         | Provider App Registration already exists for the Instance and connection method.                                                                                                                                                                                                             |
| `protected_change.not_found`                         | `5`  | `404`               | `-`         | Protected Change not found for the tenant-qualified ID.                                                                                                                                                                                                                                      |
| `protected_change.invalid_transition`                | `2`  | `400`               | `-`         | State transition is not allowed by the Protected Change state machine.                                                                                                                                                                                                                       |
| `protected_change.terminal_state`                    | `6`  | `409`               | `-`         | Protected Change is already in a terminal state.                                                                                                                                                                                                                                             |
| `protected_change.active_change_exists`              | `6`  | `409`               | `-`         | Another active Protected Change already exists for the Protected Environment.                                                                                                                                                                                                                |
| `protected_change.missing_evidence`                  | `2`  | `400`               | `-`         | Required approval evidence metadata is missing.                                                                                                                                                                                                                                              |
| `protected_change.requester_mismatch`                | `4`  | `403`               | `-`         | Acting credential is not the Protected Change requester.                                                                                                                                                                                                                                     |
| `protected_change.non_protected_environment`         | `4`  | `403`               | `-`         | Protected Changes require a Protected Environment.                                                                                                                                                                                                                                           |
| `protected_change.approval_not_authorized`           | `4`  | `403`               | `-`         | Protected delivery execution has no current authorizing approval evidence (rejected, canceled, stale-closed, or not yet approved).                                                                                                                                                           |
| `protected_change.delivery_target_mismatch`          | `4`  | `403`               | `-`         | Approval evidence was issued for a different tenant, project, Protected Environment, operation, or target than the requested protected delivery execution.                                                                                                                                   |
...
| `sync.stale_plan`                                    | `7`  | `n/a (client-side)` | `-`         | Sync Execution Revalidation found the plan no longer matches current sync configuration, App Connection state, or Explicit Provider Lookup status; re-plan before execution. No provider write starts.                                                                                       |
...
| `local_store.unavailable`                            | `7`  | `n/a (client-side)` | `required`  | No supported OS credential store is available. Local Mode fails closed instead of silently storing its root key beside ciphertext; remediation explains Linux Secret Service setup and the explicit development-only opt-in.                                                                 |
...
| `delivery_policy.not_configured`                     | `7`  | `409`               | `-`         | No Delivery Risk Policy record exists for the Project; automation resolution fails closed until one is selected (ADR-0043).                                                                                                                                                                  |
| `delivery_policy.preset_invalid`                     | `2`  | `400`               | `-`         | Preset is not one of `strict`, `balanced`, `automation_friendly`.                                                                                                                                                                                                                            |
| `delivery_policy.preset_version_unsupported`         | `2`  | `400`               | `-`         | Preset template version is missing or not supported by this build; automation fails closed.                                                                                                                                                                                                  |
| `delivery_policy.actor_invalid`                      | `4`  | `403`               | `-`         | Delivery Risk Policy configuration requires an authenticated human actor; machine and missing actors are denied (ADR-0043).                                                                                                                                                                  |
| `delivery_policy.scope_invalid`                      | `2`  | `400`               | `-`         | Tenant, project, or environment scope is missing or inconsistent for the delivery policy action.                                                                                                                                                                                             |
| `delivery_policy.preview_automation_opt_in_required` | `4`  | `403`               | `-`         | Balanced preset allows preview automation only after explicit Preview Automation Opt-In for the exact preview Environment.                                                                                                                                                                   |
| `delivery_policy.preview_automation_not_allowed`     | `4`  | `403`               | `-`         | Strict preset requires human review for preview delivery; preview automation is denied regardless of opt-in records.                                                                                                                                                                         |
| `delivery_policy.protected_approval_required`        | `4`  | `403`               | `-`         | Protected Environments always require protected delivery approval evidence; no preset grants automation there.                                                                                                                                                                               |
| `delivery_policy.opt_in_environment_invalid`         | `2`  | `400`               | `-`         | Preview Automation Opt-In applies only to non-protected preview Environments.                                                                                                                                                                                                                |
| `delivery_policy.opt_in_not_found`                   | `5`  | `404`               | `-`         | No active Preview Automation Opt-In exists for the Environment.                                                                                                                                                                                                                              |
