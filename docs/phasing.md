...
| Customer-Managed Key Custody (ADR-0050)                         | Keep ADR-0064's per-request, on-demand key-resolution seam additive so a customer-granted wrapping authority can slot in later; do not build grant management, the Custody-Locked state, customer wrapping-authority integration, or related Linear work.                                                                                        | Promote when a Hosted Instance design partner requires customer-controlled revocation as a purchase condition, after the Storage Security Gate and the single-instance-root custody path are proven in production.                                                                                    |
| Self-hosting (Self-Hosted Instance delivery)                    | The same runtime, codebase, and Instance / Instance Configuration model stay self-host-ready (ADR-0020; vision.md Non-goals: "Self-hosting in V1; the same runtime can target customer-controlled Cloudflare infra later"); do not build self-host packaging, the self-host release/update model, self-host docs, or self-host onboarding.       | Promote only after the Hosted Instance production spine is proven, the open Self-Hosted Instance (BYOC) questions in open-questions.md (insecur-the-company's access boundary on customer-operated Instances; the self-hosted release/update model) are decided, and concrete customer demand exists. |
| Crypto-erasure of expired Retained Published Version ciphertext | V1 retains an expired Retained Published Version's wrapped ciphertext as a plain row and ships no erasure path; window expiry changes rollback eligibility only, evaluated lazily at rollback request time ([adr/0076-lazy-lifecycle-expiry-and-retained-version-disposal.md](adr/0076-lazy-lifecycle-expiry-and-retained-version-disposal.md)). | Promote when a retention or deletion requirement forces destroying expired ciphertext; because shared org/project data keys mean per-version erasure is ciphertext deletion across every durable copy (including backups), not key destruction, it needs its own decision.                            |
