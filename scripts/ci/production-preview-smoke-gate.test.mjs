...
test("daily release owns the timezone-aware serialized release train", async () => {
  ...
  assert.match(
    daily,
    /production:[\s\S]*- linear_preview[\s\S]*ci_run_id:[\s\S]*deploy_sha:[\s\S]*orchestrator_sha: \$\{\{ github\.workflow_sha \}\}/u,
  );
  assert.match(
    daily,
  ...
  const smokeSecrets = daily.slice(daily.indexOf("  smoke:"), daily.indexOf("  linear_preview:"));
...
test("daily release records the exact SHA in environment-scoped Linear pipelines", async () => {
  ...
  assert.match(
    daily,
    /linear_preview:[\s\S]*- smoke[\s\S]*command: sync[\s\S]*environment_name: Preview/u,
  );
  assert.match(
    daily,
    /linear_production:[\s\S]*- production[\s\S]*command: sync[\s\S]*environment_name: Production/u,
  );
  assert.match(daily, /needs\.linear_production\.result == 'success'/u);
