...
import { parseAuditEventsBody } from "../src/console/audit.js";
import {
...
describe("audit section read over the BFF seam", () => {
  it("forwards filters and pagination to the tenant audit-events route", async () => {
    ...
    const body = await client.listAuditEvents(ORG_ID, {
      pageSize: 25,
      ...(search.cursor === undefined ? {} : { cursor: search.cursor }),
      ...(filters === undefined ? {} : { filters }),
    });
    const page = parseAuditEventsBody(body);
  ...
  it("parses empty audit pages as valid authorized reads", async () => {
    ...
    expect(parseAuditEventsBody(await client.listAuditEvents(ORG_ID, { pageSize: 25 }))).toEqual({
      events: [],
      nextCursor: null,
    });
  });
  ...
  it("fails closed on a non-member denial", async () => {
    ...
    expect(parseAuditEventsBody(await client.listAuditEvents(ORG_ID, { pageSize: 25 }))).toBeNull();
  });
});
