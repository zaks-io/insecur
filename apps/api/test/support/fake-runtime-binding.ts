 ...
 * for the private Service Binding: the API calls `env.RUNTIME.consumeGrant/.writeSecret` and the
 * call lands in the actual Runtime implementation against the same Postgres and crypto - no mock of
 * the keyring path. The cloud smoke layer drives the real binding over HTTP separately.
 */
