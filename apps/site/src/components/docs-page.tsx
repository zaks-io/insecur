 ...
 * twin so readers (human or agent) can switch formats from any page. The root SiteShell supplies
 * header and footer.
 */
export function DocsPage({ page }: { page: DocPage }) {
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-4 py-10 sm:px-6 sm:py-12 lg:flex-row">
...
function DocsSidebar({ activeSlug }: { activeSlug: string }) {
  return (
    <nav
      ...
      className="w-full shrink-0 lg:sticky lg:top-24 lg:w-56 lg:self-start"
    >
                  ...
                  <a
                    ...
                    className={
                      entry.slug === activeSlug
                        ? "block rounded-md bg-muted px-2.5 py-1.5 text-sm font-medium text-foreground"
                        : "block rounded-md px-2.5 py-1.5 text-sm text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground"
                    }
                  >
                    {entry.title}
                  </a>
