...
- Several CI gates are decided but not yet built: the Plaintext Metadata Allowlist conformance gate (ADR-0070), the role-bundle registry conformance suite (ADR-0034 amendment), and the `OPERATION_INTENT_CODES` catalog (ADR-0068). Do not claim or run them until their tickets land; do land them before feature work that depends on them. The no-plaintext canary gate `pnpm test:canary` (ADR-0069) runs in the `postgres-integration` job after `test:e2e`. The decrypt-import lint boundary (ADR-0071) is enforced via `eslint.config.ts` inside `pnpm verify`. This "decided but not yet built" list is a known merge-conflict hotspot: parallel slices that each edit it serialize-conflict (INS-168/INS-174 burned three rebases). Do not assign per-slice edits to this list; let the INS-183 doc sweep own bulk removals post-merge.
