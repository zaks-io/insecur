...
- **Verify:** `pnpm verify` (duplicate warnings + blocking ratchet, knip, actionlint, deploy/package conformance, format check, lint, typecheck, and unit-test task fan-out)
- **Duplicate scan:** `pnpm duplicates:check` (strict jscpd zero gate). CI/pre-push enforce `pnpm duplicates:ci` (ratchet, threshold 0.25%).
